...
class User(UserMixin, db.Model):
    ...
    cars = db.relationship('Car', backref='seller', lazy='dynamic')
    ...
    inquiries_received = db.relationship('Inquiry', backref='receiver', lazy=True, foreign_keys='Inquiry.receiver_id')
...
class Category(db.Model):
    ...
    cars = db.relationship('Car', backref='category', lazy='dynamic')
...
class Car(db.Model):
    ...
    inquiries = db.relationship('Inquiry', backref='car', lazy=True, foreign_keys='Inquiry.car_id')
    trade_inquiries = db.relationship('Inquiry', backref='trade_car', lazy=True, foreign_keys='Inquiry.trade_car_id')
    ...
    def generate_slug(self):
        if self.title:
            ...
            count = 1
            while Car.query.filter_by(slug=self.slug).first() is not None:
    ...
    def accept_trade(self):
        ...
        if self.status == 'Pending':
            ...
            trade_history = TradeHistory(
                ...
                offered_car_details=f"{offered_car.year} {offered_car.model}",
                requested_car_details=f"{requested_car.year} {requested_car.model}",
...
class Inquiry(db.Model):
    ...
    is_trade_offer = db.Column(db.Boolean, default=False)
    trade_car_id = db.Column(db.Integer, db.ForeignKey('cars.id'), nullable=True)
    status = db.Column(db.String(20), default='pending')  # pending, accepted, rejected
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.car_id:
            ...
            if car and car.seller_id:
                Notification.create_inquiry_notification(car.seller_id, self)
...
class Notification(db.Model):
    ...
    def create_trade_request_notification(cls, user_id, trade_request):
        ...
        notification = cls(
            ...
            message=f"New trade request: {offered_car.year} {offered_car.model} "
                   f"for your {requested_car.year} {requested_car.model}",
    ...
    def create_trade_status_notification(cls, user_id, trade_request, status):
        ...
        message = f"Trade request for {offered_car.year} {offered_car.model} "
    ...
    def create_inquiry_notification(cls, user_id, inquiry):
        ...
        notification = cls(
            user_id=user_id,
            title="New Inquiry",
            message=f"New inquiry received for your {car.year} {car.model}",
